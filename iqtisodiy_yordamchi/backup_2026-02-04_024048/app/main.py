...
try:
    ...
    init_tax_routes(app, db, User, SalesOrder, Customer, Invoice, login_required)
except Exception as e:
...
class User(db.Model):
    ...
    role_id = db.Column(db.Integer, db.ForeignKey('role.id'), default=3)  # default: user
...
class Role(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(50), unique=True, nullable=False)  # admin, manager, cashier, warehouse
    permissions = db.Column(db.JSON)  # {module: ['read', 'write', 'delete']}
...
class Product(db.Model):
    ...
    unit = db.Column(db.String(20))  # dona, kg, litr
...
class SalesOrder(db.Model):
    ...
    status = db.Column(db.String(50), default='Yangi')  # Yangi, Tasdiqlangan, Etkazilmoqda, Tugallangan
...
class PurchaseOrder(db.Model):
    ...
    total_amount = db.Column(db.Float, default=0)
    status = db.Column(db.String(50), default='Yangi')  # Yangi, Tasdiqlangan, Qabul qilindi
    items = db.relationship('PurchaseOrderItem', backref='purchase_order', lazy=True, cascade='all, delete-orphan')


class PurchaseOrderItem(db.Model):
    """Sotib olish buyurtmasi elementi"""
    id = db.Column(db.Integer, primary_key=True)
...
class Inventory(db.Model):
    ...
    status = db.Column(db.String(50), default='Boshlangan')  # Boshlangan, Tugallangan
...
class Invoice(db.Model):
    ...
    status = db.Column(db.String(50), default='Yangi')  # Yangi, Qisman to'langan, To'langan


class Payment(db.Model):
    """To'lovlar (payment gateway callbacks)"""
    id = db.Column(db.Integer, primary_key=True)
...
class CashTransaction(db.Model):
    ...
    transaction_type = db.Column(db.String(50))  # Kirim, Chiqim
    amount = db.Column(db.Float, nullable=False)
    description = db.Column(db.String(200))
    reference = db.Column(db.String(100))  # Invoice, Order number
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class Expense(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    number = db.Column(db.String(50), unique=True, nullable=False)
    category = db.Column(db.String(50))  # Rent, Salary, Utilities, Marketing
    amount = db.Column(db.Float, nullable=False)
    description = db.Column(db.String(200))
    expense_date = db.Column(db.DateTime, default=datetime.utcnow)
    payment_method = db.Column(db.String(50))  # Cash, Bank
    status = db.Column(db.String(50), default='Yangi')  # Yangi, Tasdiqlangan, To'langan
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class JournalEntry(db.Model):
    ...
    amount = db.Column(db.Float, nullable=False)
    status = db.Column(db.String(50), default='Yangi')  # Yangi, Tasdiqlangan
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class AIAssistant(db.Model):
    """AI Yordamchi - Savol-Javob Tavsiya"""
    id = db.Column(db.Integer, primary_key=True)
    question = db.Column(db.String(500), nullable=False)
    answer = db.Column(db.Text)
    category = db.Column(db.String(50))  # business, product, sales, inventory
...
class OfflineSync(db.Model):
    """Offline rejim uchun sinxronizatsiya"""
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    device_id = db.Column(db.String(100))
    data_snapshot = db.Column(db.JSON)  # Offline ma'lumotlar
    last_sync = db.Column(db.DateTime, default=datetime.utcnow)
    pending_changes = db.Column(db.JSON)  # Sinxron qilinmagan o'zgarishlar
...
class Report(db.Model):
    ...
    report_type = db.Column(db.String(50))  # sales, inventory, financial, cash
    created_by_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    data = db.Column(db.JSON)
    chart_type = db.Column(db.String(50))  # bar, pie, line
    export_format = db.Column(db.String(50))  # xlsx, pdf
...
class DataIngest(db.Model):
    """Ingest qilingan ma'lumot snapshoti"""
    id = db.Column(db.Integer, primary_key=True)
    source = db.Column(db.String(100))  # mobile, desktop, csv, api
...
class ValidationReport(db.Model):
    """Ma'lumot validatsiya hisobotlari"""
    id = db.Column(db.Integer, primary_key=True)
...
class Forecast(db.Model):
    """Oddiy prognoz natijalari (moving average)"""
    id = db.Column(db.Integer, primary_key=True)
