...
def init_ocr_routes(app, db, User, login_required):
    ...
    def extract_text():
        ...
        try:
            ...
            logger.info(f"Matn ajratildi: {file.filename}")
    ...
    def extract_invoice():
        ...
        try:
            ...
            # Faylni saqlash
            filename = secure_filename(f"invoice_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{file.filename}")
            file_path = os.path.join(UPLOAD_FOLDER, filename)
            file.save(file_path)
            ...
            logger.info(f"Hisob-faktura ajratildi: {file.filename}")
    ...
    def scan_and_save():
        ...
        try:
            ...
            # Faylni saqlash
            filename = secure_filename(f"scanned_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{file.filename}")
            file_path = os.path.join(UPLOAD_FOLDER, filename)
            file.save(file_path)
            ...
            logger.info(f"Hujjat saqlandi: {file.filename}")
